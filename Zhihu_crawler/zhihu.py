...
"""
    ...
    def get_board(self) -> list:
        ...
        """
        resp_zhihu_hot = requests.get("https://www.zhihu.com/hot", headers = self.settings["headers"])
        html_zhihu_hot = BS(resp_zhihu_hot.text, 'lxml')
        secs = html_zhihu_hot.find_all('section', class_ = 'HotItem')
        hot_list = []
        for idex, sec in enumerate(secs):
            if sec.find('h2', class_ = 'HotItem-title') is not None:
                heat_title = sec.find('h2', class_ = 'HotItem-title').text
            else:
                heat_title = None
            if sec.find('div', 'HotItem-metrics HotItem-metrics--bottom') is not None:
                heat_value = sec.find('div', 'HotItem-metrics HotItem-metrics--bottom').text 
                heat_share = heat_value.replace('\u200b', ',')
                heat = heat_share.split(',')[0]
            elif sec.find('div', 'HotItem-metrics') is not None:
                heat_value = sec.find('div', 'HotItem-metrics').text 
                heat_share = heat_value.replace('\u200b', ',')
                heat = heat_share.split(',')[0]
            else:
                heat = None
            if sec.find('p', class_ = 'HotItem-excerpt') is not None:
                heat_excerpt = sec.find('p', class_ = 'HotItem-excerpt').text
            else:
                heat_excerpt = None
            if sec.find('div', class_ = 'HotItem-content').find('a') is not None:
                url = sec.find('div', class_ = 'HotItem-content').find('a').get('href')
                qid = url.split('/')[-1]
            else:
                url = None
                qid = None
    ...
    def get_question(self, qid: int) -> dict:
        ...
        q_soup = BS(resp.text, 'lxml')
        json_str_initialData = q_soup.find('script', id = 'js-initialData').text
        json_initialData = json.loads(json_str_initialData)
        info = json_initialData['initialState']['entities']['questions'][f'{qid}']
        return {
